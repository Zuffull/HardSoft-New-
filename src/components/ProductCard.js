  ...
  return (
    <div className="product-card">
      ...
      <div className="product-info">
        ...
        <div className="product-meta">
          <p className="product-sku">Код: {product.sku}</p>
          <div className="product-rating" title={`Рейтинг: ${product.averageRating || product.rating || 0}/5`}>
            {renderStars(product.averageRating || product.rating || 0)}
          </div>
        </div>
        <ul className="product-specs">
          <li>Процесор: {product.attributes?.Процесор || product.processor || "Не вказано"}</li>
          <li>Відеокарта: {product.attributes?.Відеокарта || product.gpu || "Не вказано"}</li>
          <li>Тип пам'яті: {product.attributes?.["Тип пам'яті"] || product.memoryType || "Не вказано"}</li>
          <li>Оперативна пам'ять: {product.attributes?.["Оперативна пам'ять"] || product.ram || "Не вказано"}</li>
          <li>Материнська плата: {product.attributes?.["Материнська плата"] || product.motherboard || "Не вказано"}</li>
          <li>Накопичувач: {product.attributes?.Накопичувач || product.storage || "Не вказано"}</li>
        </ul>
        <p className="product-price">{Number(product.price).toLocaleString('uk-UA')} ₴</p>
        <div className="product-actions">
          <a href="#" className={`product-button ${product.stock === 0 ? 'disabled' : ''}`}>
